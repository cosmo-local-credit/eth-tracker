...
const (
	testRPCEndpoint = "https://r3-celo.grassecon.org"
	testChainID     = 42220
)
...
func TestRPC_GetBlocks(t *testing.T) {
	...
	blockNumbers := make([]uint64, 100)
	for i := range blockNumbers {
		blockNumbers[i] = 26_385_000 + uint64(i)
	}
...
func TestRPC_GetBlocks_LessThanBatchSize(t *testing.T) {
	rpcFetcher, err := setupEthRPC()
	require.NoError(t, err)
	...
	blocks, err := rpcFetcher.GetBlocks(ctx, blockNumbers)
	require.NoError(t, err)
	require.Len(t, blocks, len(blockNumbers))
