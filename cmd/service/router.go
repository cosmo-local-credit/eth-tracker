...
import (
	...
	"github.com/cosmo-local-credit/eth-tracker/internal/handler"
...
func bootstrapEventRouter(cacheProvider cache.Cache, pubCB router.Callback, custodialAddr string) *router.Router {
	...
	if custodialAddr != "" {
		router.RegisterInputDataRouteForContract("4420e486", custodialAddr, handler.HandleCustodialRegistrationInputData())
	}
