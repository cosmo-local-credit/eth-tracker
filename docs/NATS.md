...
The custodial stack heavily utilizes NATS JetStream to emit all relevant events to any connected client. Primarily there are 2 main types of events:
...
* Explicitly ACK a message after processing it. This allows NATS to redeliver the message in case of a failure on the integrating client.
...
* `contractAddress`: The contract address which emitted the specific chain event when success is true, otherwise it is the contract address with which the signer interacted when success is false.
* `success`: True if successfully mined, false if the transaction reverted.
* `transactionType`: The description of the transaction type. This will also match the subject name.
* `index`: The log's index within the block for log-emitting events. `0` for reverted input-data events (no log emitted). `18446744073709551615` (max uint) for `CONTRACT_CREATION` events.
* `payload`: Additional details in relation to the transaction type.

Additionally, the NATS message ID is always in the format: `transactionHash:logIndex` — this allows the client to deduplicate if necessary.

Individual transaction types and their payload are described below. The payload is consistent with both successful and reverted transactions. In certain reverted cases, the zero value of the type is used as the value for certain payload keys.

#### `TRACKER.CONTRACT_CREATION`

This is a special event that is emitted when an EOA account creates a contract.
...
This event is emitted when an account is created, registered and topped up with gas on-chain by `eth-custodial`. If successful, it signals that the custodial account is ready for use.
...
This event is emitted when gas is requested from the faucet.

```json
{
    "recipient": String, // Account address
    "token": String,     // Faucet gifted token address
    "amount": String     // Token amount gifted ("0" for reverted transactions)
}
```

#### `TRACKER.INDEX_ADD`

This event is emitted when an address is added to any GE index.
...
This event is emitted when an address is removed from any GE index.
...
```json
{
    ...
    "holder": String, // Account the limit applies to
...
This event is emitted when a contract's ownership is transferred to another address.

```json
{
    "previousOwner": String, // Previous owner account address
    "newOwner": String       // New owner account address
}
```

#### `TRACKER.POOL_DEPOSIT`

This event is emitted when a token is deposited into any swap pool. The amount values are in relation to the token's decimal precision.
...
#### `TRACKER.QUOTER_UPDATED`

This event is emitted when the quoter address is updated on a pool contract.

```json
{
    "newQuoter": String // New quoter contract address
}
```

#### `TRACKER.SEAL_STATE_CHANGE`

This event is emitted when the seal state changes for any GE related smart contract.

```json
{
    "final": Boolean,    // Final state
    "sealState": String  // State value as a big.Int string
}
```

#### `TRACKER.TOKEN_APPROVE`

This event is emitted when a spend approval is requested on a token.
...
This event is emitted when tokens are burnt.
...
This event is emitted when tokens are minted.

```json
{
    "tokenMinter": String, // Minter address
    "to": String,          // Mint receiver address
    "value": String        // Value of the mint
}
```

#### `TRACKER.TOKEN_TRANSFER`

This event is emitted when tokens are transferred via the `transfer` call.

```json
{
    "from": String,  // Sender address
    "to": String,    // Receiver address
    "value": String  // Value of the transfer
}
```

#### `TRACKER.TOKEN_TRANSFER_FROM`

This event is emitted when tokens are transferred on behalf of another address via the `transferFrom` call.

```json
{
    "from": String,    // Token sender address
    "to": String,      // Token receiver address
    "spender": String, // Address that initiated the transfer
    "value": String    // Value of the transfer
}
```
...
2. `IN_NETWORK`: Signed tx successfully dispatched to the RPC node.
3. `SUCCESS`: Signed tx successfully mined on the chain.
...
1. `PENDING`: Successfully signed and stored in the OTX store.
2. `IN_NETWORK`: Signed tx successfully dispatched to the RPC node.
3. `REVERTED`: Signed tx reverted on chain.
