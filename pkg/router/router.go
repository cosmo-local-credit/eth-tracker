...
type (
	...
	Router struct {
		...
		contractInputHandlers   map[string]InputDataEntry
...
func New(callbackFn Callback) *Router {
	return &Router{
		...
		contractInputHandlers:   make(map[string]InputDataEntry),
...
func (r *Router) RegisterInputDataRouteForContract(selector string, contractAddress string, handlerFunc InputDataHandlerFunc) {
	key := selector + ":" + contractAddress
	r.contractInputHandlers[key] = InputDataEntry{
		Signature:   selector,
		HandlerFunc: handlerFunc,
	}
}
...
func (r *Router) ProcessInputData(ctx context.Context, payload InputDataPayload) error {
	...
	selector := payload.InputData[:8]

	contractKey := selector + ":" + payload.ContractAddress
	if handler, ok := r.contractInputHandlers[contractKey]; ok {
		return handler.HandlerFunc(ctx, payload, r.callbackFn)
	}

	if handler, ok := r.inputDataHandlers[selector]; ok {
		return handler.HandlerFunc(ctx, payload, r.callbackFn)
	}
